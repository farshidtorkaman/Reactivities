    ...
    return (
        <Grid>
            <Grid.Column width={10}>
                <Segment clearing >
                    <FinalForm onSubmit={handleFinalFormSubmit} validate={validate} initialValues={activity} render={({ handleSubmit, invalid, pristine }) => (
                        <Form onSubmit={handleSubmit} loading={loading}>
                            ...
                            <FormGroup widths='equal'>
                                <Field type='datetime-local' placeholder='Date' date={true} name='date' value={activity.date} component={DateInput} />
                                <Field type='datetime-local' placeholder='Time' time={true} name='time' value={activity.time} component={DateInput} />
                            </FormGroup>
                            ...
                            <Button floated='right' loading={submitting} positive type='submit' content='Submit' disabled={loading || invalid || pristine} />
                            <Button onClick={activity.id ? () => history.push(`/activity/${activity.id}`) :  () => history.push('/activities')} floated='right' type='button' disabled={loading} content='Cancel' />
                        </Form>
                    )} />
                </Segment>
            </Grid.Column>
